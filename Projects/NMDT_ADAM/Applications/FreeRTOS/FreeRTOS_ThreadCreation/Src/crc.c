...
/* 对原始数据转义后计算crc16 */
unsigned short CalcCRC_ESC(unsigned char *data, unsigned int size)
{
	unsigned int i;
	unsigned char crc_dat = 0;
	unsigned short crc = 0;

	for (i = 0; i < size - 1; i++)
	{
		//解帧过程如果遇到连续两个字节为 0xA6, 0x02 ,则反转义为 0xA5 
		if ((data[i] == 0xA6) && (data[i + 1] == 0x02))
		{
			crc_dat = 0xA5;
			i += 1;
		}
		//解帧过程如果遇到连续两个字节为 0xA6, 0x01 ,则反转义为 0xA6
		else if ((data[i] == 0xA6) && (data[i + 1] == 0x01))
		{
			crc_dat = 0xA6;
			i += 1;
		}
		//解帧过程如果遇到连续两个字节为 0x5B, 0x02 ,则反转义为 0x5A
		else if ((data[i] == 0x5B) && (data[i + 1] == 0x02))
		{
			crc_dat = 0x5A;
			i += 1;
		}
